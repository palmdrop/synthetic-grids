...
const encodedFragmentProgram = encodedFragmentPrograms[Math.floor(Math.random() * encodedDisplacementPrograms.length)];
console.log(encodedFragmentProgram);
  ...
  return {
    fuseShader: makeSampleFuseShader(
      ...
      `
        ...
        float n = length(samplerColor) * 1.0;
      `,
      textures.slice(1),
      textures[0]
    ),
  ...
  const vertexSourceData = {
    ...
    main: `
      ...
      vec3 samplePosition = vec3(
        position.x * 10.0,
        position.y * 10.0,
        position.z + animationTime * 2.0
      );

      float n = getOffset(
        samplePosition, false
      );

      vec3 centerSamplePosition = vec3(
        0.0, 
        // position.y + animationTime,
        0.0,
        0.0
      );

      float cn = getOffset(
        centerSamplePosition, false
      );
