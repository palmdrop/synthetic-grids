<script lang="ts">
	...
	import './global.css';
  // import encodedGateProgram from './graphics/three/synthetics/programs/gate1.json';
  // '../../programs/gate1.json';
  ...
  import { getStructureEchoesSpace, spaceMetadata } from './graphics/three/synthetics/spaces/structure-echoes/structureEchoesSpace';
  ...
  onMount(() => {
    Promise.all([
      // decodeProgram(encodedGateProgram as unknown as EncodedProgram)
      Promise.resolve()
    ])
      .then(([mainProgram]) => {
        scene = new SyntheticGrids(canvas, getStructureEchoesSpace, {
          spaceMetadata,
          // program: mainProgram
        });
        scene.resize();
